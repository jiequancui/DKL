    ...
    if distance == 'l_inf':
        ...
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss_kl = dkl_loss(model(x_adv), model(x_natural), CLASS_PRIOR=CLASS_PRIOR, alpha=alpha, beta=beta, gamma=gamma)
...
def train(model, train_loader, optimizer, epoch, awp_adversary, start_wa, tau_list, exp_avgs, weight=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.aug != 'basic':
            ...
            loss_robust = dkl_loss(logits_aa, logits_na, CLASS_PRIOR=sample_weight[:bt], alpha=args.alpha, beta=args.beta, gamma=args.gamma) + args.aug_weight * \
                          dkl_loss(logits_ab, logits_nb, CLASS_PRIOR=sample_weight[bt:], alpha=args.alpha, beta=args.beta, gamma=args.gamma)
        else:
            loss_robust = dkl_loss(logits_adv, logits_nat, CLASS_PRIOR=sample_weight, alpha=args.alpha, beta=args.beta, gamma=args.gamma)
